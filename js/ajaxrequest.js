(function () {
    ...
    AjaxRequest.prototype.makeAjaxObject = function () {
        if (window.XMLHttpRequest) {
            ...
            this.xmlhttp[this.objIndex] = new ActiveXObject(
                "Microsoft.XMLHTTP"
            );
        }
    ...
    AjaxRequest.prototype.request = function (
        ...
        const callbackFunc = requestOptions.callbackFunction
            ? requestOptions.callback
            : null;
        const callbackFuncContext = requestOptions.hasOwnProperty[
            "callbackContext"
        ]
            ? requestOptions.callbackContext
            : null;
        const progress = requestOptions.hasOwnProperty["progressCallback"]
            ? requestOptions.progressCallback
            : null;

        this.apiCall(target, callbackFunc, data, callbackFuncContext, progress);
    };

    //depreciated but dont remove, request() instead
    AjaxRequest.prototype.apiCall = function (
        target,
        callback = null,
        data = null,
        cbcontext = null,
    ...
    AjaxRequest.prototype.postRequest = function (
        target,
        data = null,
        options = null
    ) {
        ...
        const callbackFuncContext =
            options.callbackContext != "undefined"
        ...
        function makeRequest() {
            ...
            if (context.options.requestHeader) {
                if (data instanceof FormData) {
                    //do not set request header for form data obj
                } else {
            ...
            if (typeof progress === "function") {
                xmlhttp.upload.addEventListener(
                    ...
                    function (e) {
                        ...
                        if (cbcontext == null) progress(progressData);
    ...
    function waitForResponse(index) {
        ...
        context.xmlhttp[index].onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
                ...
                result =
                    context.options.resultType == "xml"
                        ? this.responseXML
                        : context.options.resultType == "json"
                ...
                if (context.callbackContext[index] === null) {
                    ...
                    let arr = [result];
                    context.callback[index].apply(
                        context.callbackContext[index],
                        arr
                    );
                }
